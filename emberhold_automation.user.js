...
(function () {
  ...
  function autoMorale(state) {
    ...
    const normalTarget = Number.isFinite(liveRate) && marginal > 0
      ? Math.max(0, Math.ceil((0.025 - (liveRate - performers * marginal)) /
        marginal - 1e-9))
  ...
  function autoJobs(state, demand) {
    ...
    if (!foodEmergency && !explorerReclaimed && defs.explorer &&
        jobUnlocked(defs.explorer) && count('explorer') < 1 &&
  ...
  function autoPower(state, demand) {
    ...
    const siteOutput = site => {
      ...
      if (typeof site.job === 'string' && Object.hasOwn(state.res || {}, site.job)) {
        return site.job;
      }
      if (site.job && typeof site.job === 'object') {
        if (site.job.res || site.job.resource || site.job.output) {
          return site.job.res || site.job.resource || site.job.output;
        }
      }
      const jobDef = Array.isArray(jobs)
        ? jobs.find(job => job.id === jobId)
        : jobs[jobId];
      if (jobDef?.res || jobDef?.resource || jobDef?.output) {
        return jobDef.res || jobDef.resource || jobDef.output;
      }
